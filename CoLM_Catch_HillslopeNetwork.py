...
class hillslope_network_info_type(object):
    nhru = 0
    ihru = None # location of HRU in global vector "landhru"
    indx = None # index of HRU
    area = None # area of HRU [m^2]
    agwt = None # water area only including (patchtype <= 2) [m^2]
    hand = None # height above nearest drainage [m]
    elva = None # elevation [m]
    plen = None # average drainage path length to downstream HRU [m]
    flen = None # interface length between this and downstream HRU [m]
    inext= None # location of next HRU in this basin


class CoLM_Catch_HillslopeNetwork(object):
