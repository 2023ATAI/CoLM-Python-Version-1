    ...
    def landurban_build(self, lc_year, gurban,landelm,landhru):
        ...
        if self.mpi.p_is_io:
            ...
            if self.nl_colm['DEF_URBAN_type_scheme'] == 1:
                data_urb_class = self.readin.read_5x5_data(dir_urban, suffix, gurban, 'URBAN_DENSITY_CLASS', data_urb_class)
        ...
        if self.mpi.p_is_worker:
            if self.numpatch > 0:
                ...
                numpatch_ = self.numpatch + np.count_nonzero(self.landpatch.settyp == self.var_global.URBAN) * (self.var_global.N_URB - 1)
            ...
            for ipatch in range( self.numpatch ):
                if self.landpatch.settyp[ipatch] == self.var_global.URBAN:
                    ...
                    ard = AggregationRequestData(self.mpi, self.mesh.mesh, self.gblock.pixel)
                    ibuff = ard.aggregation_request_data(self.landpatch, ipatch+1, gurban, zip=False, data_i4_2d_in1=data_urb_class)
                    if self.nl_colm['DEF_URBAN_type_scheme'] == 1:
            ...
            if self.numurban > 0:
                ...
                self.landurban.eindex = np.array([self.landpatch.eindex[i] for i in range(len(self.landpatch.settyp)) if self.landpatch.settyp[i] == self.URBAN])
                self.landurban.ipxstt = np.array([self.landpatch.ipxstt[i] for i in range(len(self.landpatch.settyp)) if self.landpatch.settyp[i] == self.URBAN])
                self.landurban.ipxend = np.array([self.landpatch.ipxend[i] for i in range(len(self.landpatch.settyp)) if self.landpatch.settyp[i] == self.URBAN])
                self.landurban.ielm = np.array([self.landpatch.ielm[i] for i in range(len(self.landpatch.settyp)) if self.landpatch.settyp[i] == self.URBAN])
