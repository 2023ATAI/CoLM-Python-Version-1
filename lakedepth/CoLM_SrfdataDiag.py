...
class CoLm_SrfdataDiag(object):
    def __init__(self,nl_colm, mpi, gblock) -> None:
        ...
        self.gdiag = Grid_type(nl_colm, gblock, mpi)
        self.srf_concat = GridConcatType()
        self.srf_concat.set(self.gdiag, gblock)
        self.m_elm2diag = MappingPset2Grid()
        self.m_patch2diag = MappingPset2Grid()
        self.m_pft2diag = MappingPset2Grid()
        self.m_urb2diag = MappingPset2Grid()
    
    def srfdata_diag_init(self, dir_landdata, landpatch,landelm, elm_patch,landpft,landurban,hru_patch,pctshrpch,N_land_classification):
        landdir = dir_landdata.strip() + '/diag/'
        if self.mpi.p_is_master:
            os.makedirs(landdir.strip())

        self.srf_concat.set(self.gdiag)

        self.m_elm2diag.build(landelm, self.gdiag)

        if not self.nl_colm['CROP']:
            self.m_patch2diag.build(landpatch, self.gdiag)
        else:
            self.m_patch2diag.build(landpatch, self.gdiag, pctshrpch)

        if self.nl_colm['LULC_IGBP_PFT'] or self.nl_colm['LULC_IGBP_PC']:
            self.m_pft2diag.build(landpft, self.gdiag)

        if self.nl_colm['URBAN_MODEL']:
            self.m_urb2diag.build(landurban, self.gdiag)
    ...
    def srfdata_map_and_write(self, vsrfdata, settyp, typindex, m_srf, spv, filename, dataname, compress, write_mode=None, lastdimname=None, lastdimvalue=None):
        wmode = 'one'
        ...
        if wmode == 'one':
            if self.mpi.p_is_master:
                ...
                if lastdimname is not None and lastdimvalue is not None:
                    ...
                    self.netfilencio_write_serial_time(filename.strip(), dataname.strip(), ilastdim, vdata, 'TypeIndex', 'lon', 'lat', lastdimname.strip(), compress)
                else:
                    self.netfilencio_write_serial(filename.strip(), dataname.strip(), vdata, 'TypeIndex', 'lon', 'lat', compress)
            ...
            if self.mpi.p_is_io:
                for iblkme in range(self.gblock.nblkme):
                    ...
                    if lastdimname is not None and lastdimvalue is not None:
                        ...
                        self.netfilencio_write_serial_time(fileblock, dataname, ilastdim, wdata.blk[iblk, jblk].val, 'TypeIndex', 'lon', 'lat', lastdimname, compress)
                    else:
                        self.netfilencio_write_serial(fileblock, dataname, wdata.blk[iblk, jblk].val, 'TypeIndex', 'lon', 'lat', compress)
